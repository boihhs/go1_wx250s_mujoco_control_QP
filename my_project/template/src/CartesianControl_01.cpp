...
Eigen::VectorXd LegTorque_quadruped(const std::vector<Eigen::Vector3d>& F, const Eigen::VectorXd& q, double t) {
    ...
    std::tie(J1,p1) = computeLegJacobianAndPosition(q.segment(0,3), 1);
    ...
    std::tie(J3,p3) = computeLegJacobianAndPosition(q.segment(6,3), 3);
    Eigen::Vector3d tau3 = J3.transpose() * F[2];

    std::tie(J4,p4) = computeLegJacobianAndPosition(q.segment(9,3), 4);
...
Eigen::VectorXd CartesianControl_01(double t, const Eigen::VectorXd& xin, const Eigen::VectorXd& q, const Eigen::VectorXd& xdes, const Eigen::VectorXd& fpfv, const Eigen::VectorXd& qArm) {
    ...
    double t_start = 0;
    double dt = .04;
    Eigen::Matrix3d R = eul2rotm(xin.segment(0,3).reverse()); // z, y, x

    double roll_act = xin(0);
    double pitch_act = xin(1);
    double yaw_act = xin(2);
    ...
    double vx_act = xin(9);
    double vy_act = xin(10);

    double lift_height = 0.1;

    Eigen::Vector3d fpFL = fpfv.segment(0, 3);
    Eigen::Vector3d fpFR = fpfv.segment(6,3);
    Eigen::Vector3d fpRL = fpfv.segment(12,3);
    Eigen::Vector3d fpRR = fpfv.segment(18,3);

    Eigen::Vector3d fvFL = fpfv.segment(3,3);
    Eigen::Vector3d fvFR = fpfv.segment(9,3);
    Eigen::Vector3d fvRL = fpfv.segment(15,3);
    Eigen::Vector3d fvRR = fpfv.segment(21,3);
    ...
    Eigen::Vector3d p_COM = xin.segment(3,3) + R * COM_b;

    Eigen::VectorXd tau;


    if (t > t_start and t < time_stop) {
        double t_cycle = std::fmod(t - t_start, gaitcycle / 2.0);
